...
class Main{
    static async notificationUserThreeSix(){
        connectToDatabase()
        .then((connection) => {
          connection.query('SELECT COUNT(CONSULTATION_ID) as rows FROM CONSULTATIONS WHERE STATUS = "1" AND SCHEDULE IN ("3","5")', (queryError, results) => {
            if (queryError) {
